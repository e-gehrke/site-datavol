...
import { TICKER_DATA, FORWARD_DATA } from '@/lib/data'
...
function HeroChart() {
  ...
  const yTicks = [5.2, 5.5, 5.8, 6.1], xTicks = [0, 6, 12, 24, 36]
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      {yTicks.map(t => <line key={'y'+t} x1={PAD.l} y1={ys(t)} x2={W-PAD.r} y2={ys(t)} stroke="rgba(166,166,166,.12)" strokeDasharray="2 4" />)}
      {xTicks.map(t => <line key={'x'+t} x1={xs(t)} y1={PAD.t} x2={xs(t)} y2={H-PAD.b} stroke="rgba(166,166,166,.08)" />)}
      ...
      {xTicks.map(t => <text key={'xl'+t} x={xs(t)} y={H-PAD.b+16} textAnchor="middle" fill="#A6A6A6" fontFamily="IBM Plex Mono" fontSize="10" letterSpacing="1">{t===0?'SPOT':t+'M'}</text>)}
    ...
    <div style={{ background: 'var(--black)', color: 'var(--off)', minHeight: '100vh' }}>
      ...
      <header className="pub-header">
        <div className="wrap-wide">
          <div className="header-inner">
            <Lockup size={32} fontSize={26} dot="lime" />
            <nav>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <Link href="/dashboard" className="btn btn-quiet btn-sm" style={{ color: 'var(--cinza)', borderColor: 'var(--rule-dark-strong)' }}>Acesso demo →</Link>
              <button onClick={() => setLoginOpen(true)} className="btn btn-lime btn-sm">Entrar</button>
            </div>
          </div>
        </div>
      </header>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,1fr)', gap: 56, alignItems: 'center' }} className="hero-grid">
            <div>
              ...
              <h1 className="display-lg" style={{ color: 'var(--off)', maxWidth: '14ch', textTransform: 'uppercase' }}>
                O banco oferece o preço.<br/><span style={{ color: 'var(--lime)' }}>DATA.VOL</span> mostra o cálculo.
              </h1>
              ...
              <p style={{ fontFamily: 'var(--display)', fontSize: 'clamp(20px,2vw,28px)', lineHeight: 1.05, color: 'var(--lime)', marginTop: 22, maxWidth: '36ch', paddingTop: 22, borderTop: '1px solid rgba(198,255,77,.25)' }}>
                O usuário entende o preço e decide com mais critério.
              </p>
              <div style={{ display: 'flex', gap: 10, marginTop: 32, flexWrap: 'wrap' }}>
                <button onClick={() => setLoginOpen(true)} className="btn btn-lime">Entrar →</button>
                <Link href="/dashboard" className="btn btn-ghost on-dark">Ver demo</Link>
            ...
            <div className="panel" style={{ padding: 0, background: 'var(--black-2)', border: '1px solid var(--rule-dark-strong)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: '1px solid var(--rule-dark)' }}>
              ...
              <div style={{ padding: 8 }}><HeroChart /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', borderTop: '1px solid var(--rule-dark)' }}>
                {[
                  { l: 'Spot',       v: '5,182', d: '+0,34%',          up: true,  dim: false },
                  { l: 'DV ref 12M', v: '5,518', d: 'carrego 6,49% a.a.', up: false, dim: true },
                  { l: 'Banco 12M',  v: '5,560', d: '+76 bps',         up: false, dim: false },
                ].map((k, i) => (
                  <div key={i} style={{ padding: '14px 16px', borderRight: i < 2 ? '1px solid var(--rule-dark)' : 'none' }}>
                    <p className="label-sm" style={{ color: 'var(--cinza-3)', marginBottom: 6 }}>{k.l}</p>
                    <p className="mono" style={{ fontSize: 22, lineHeight: 1, color: 'var(--off)', marginBottom: 4 }}>{k.v}</p>
                    <p className="mono" style={{ fontSize: 10, letterSpacing: 1, color: k.dim ? 'var(--cinza)' : k.up ? 'var(--up)' : 'var(--down)' }}>{k.d}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section style={{ padding: '80px 0', background: 'var(--off)', color: 'var(--grafite)' }} id="produto">
        <div className="wrap-wide">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 40, marginTop: 48 }}>
            <p className="lead" style={{ color: 'var(--cinza-3)' }}>Existem empresas que lidam com dólar, fluxo internacional e risco cambial relevante, mas não possuem tesouraria estruturada, orçamento para terminais Bloomberg, nem equipe capaz de construir modelos confiáveis em Excel.</p>
            <p className="lead" style={{ color: 'var(--cinza-3)' }}>Essas empresas acabam aceitando preços de dealers sem conseguir decompor curva, spread, premissa ou custo implícito. <strong style={{ color: 'var(--grafite)' }}>DATA.VOL existe para reduzir essa assimetria.</strong></p>
            <p className="lead" style={{ color: 'var(--cinza-3)' }}>A complexidade fica no motor. A clareza fica na interface.</p>
          </div>
          <div style={{ marginTop: 60, paddingTop: 36, borderTop: '1px solid rgba(0,0,0,.12)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 32, flexWrap: 'wrap' }}>
            <p style={{ fontFamily: 'var(--display)', fontSize: 'clamp(28px,3.6vw,48px)', lineHeight: 1.04, maxWidth: '22ch' }}>
        ...
        <div className="wrap-wide">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16, marginBottom: 36 }}>
            <div>
              ...
              <h2 className="display-md" style={{ color: 'var(--off)', maxWidth: '20ch' }}>Uma assinatura.<br/>Todos os instrumentos.</h2>
            </div>
            <p className="lead-sm" style={{ color: 'var(--cinza)', maxWidth: '40ch' }}>Sem trilhas BASIC/PRO. Sem aulas. Sem conteúdo. Ferramenta operacional, atualizada com dados de mercado.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 16 }}>
            {[
              ...
              { tit: 'Curvas de juros',     sub: 'DI · Pré · Cupom',           s: 'beta', d: 'Vértices DI futuro, estrutura a termo e leitura do mercado de taxa.' },
      ...
      <section style={{ padding: '80px 0', background: 'var(--off)', color: 'var(--grafite)' }} id="para-quem">
        <div className="wrap-wide">
          ...
          <h2 className="display-md" style={{ maxWidth: '22ch' }}>Tesouraria sem o custo<br/>de uma tesouraria.</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 56, marginTop: 48 }} className="alvo-grid">
            <div>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 14 }}>
                {['Dono ou gestor de empresa de comércio exterior (importação ou exportação)','CFO ou financeiro de empresa pequena/média com exposição em dólar','Escritório de investimento que atende empresas e precisa validar preço bancário','Assessor que precisa entender estrutura de preço antes de conversar com cliente ou banco','Profissional de backoffice que opera câmbio, NDF, swap ou hedge cambial'].map((line, i) => (
            ...
            <div style={{ background: 'var(--grafite)', color: 'var(--off)', padding: 32, borderRadius: 4, backgroundImage: 'linear-gradient(rgba(166,166,166,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(166,166,166,.05) 1px,transparent 1px)', backgroundSize: '24px 24px' }}>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10, fontFamily: 'var(--mono)', fontSize: 13 }}>
                {['Plataforma de day trade','Home broker','Sala de calls ou canal de recomendação','Consultoria individual de investimento','Curso de educação financeira','Promessa de retorno'].map((line, i) => (
                  <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, color: 'var(--cinza)' }}>
                    <span style={{ width: 10, height: 1.5, background: 'var(--cinza)', flexShrink: 0 }} />{line}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section style={{ padding: '80px 0', borderTop: '1px solid var(--rule-dark)' }} id="assinatura">
        <div className="wrap-wide">
          ...
          <h2 className="display-md" style={{ color: 'var(--off)', maxWidth: '22ch' }}>Um plano.<br/>Recorrência por utilidade.</h2>
          <p className="lead-sm" style={{ color: 'var(--cinza)', marginTop: 16, maxWidth: '52ch' }}>Sem freemium, sem trial obscuro, sem upgrades disfarçados.</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 16, marginTop: 36 }} className="plan-grid">
            <div className="panel" style={{ padding: 36, borderColor: 'var(--lime)' }}>
              ...
              <ul style={{ listStyle: 'none', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 24px', marginTop: 22, padding: '22px 0', borderTop: '1px solid var(--rule-dark)', borderBottom: '1px solid var(--rule-dark)' }}>
                {['Curva dólar forward ao vivo','Curvas de juros (DI futuro)','NDF e swaps','Comparador banco × referência','Histórico ilimitado de consultas','Biblioteca técnica','Até 3 usuários','Disclaimer e auditabilidade'].map((line, i) => (
                  <li key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 10, fontSize: 13, color: 'var(--off)' }}>
                    <span style={{ color: 'var(--lime)', fontFamily: 'var(--mono)', fontSize: 10 }}>✓</span>{line}
                  </li>
                ))}
              </ul>
              <div style={{ display: 'flex', gap: 10, marginTop: 22, flexWrap: 'wrap' }}>
                <button onClick={() => setLoginOpen(true)} className="btn btn-lime">Solicitar acesso →</button>
                <Link href="/dashboard" className="btn btn-ghost on-dark">Ver dentro do produto</Link>
              </div>
            </div>
            <div className="panel" style={{ padding: 28, background: 'transparent', borderStyle: 'dashed', borderColor: 'var(--rule-dark-strong)' }}>
              ...
              <p className="lead-sm" style={{ color: 'var(--off)', marginBottom: 20 }}>Mais de 3 usuários, integrações, dados via API ou cobertura sob medida.</p>
              <p style={{ fontSize: 13, color: 'var(--cinza)', lineHeight: 1.6, marginBottom: 24 }}>Conversamos com a empresa, mapeamos o uso e calculamos o plano. Sem mensalidade tabelada.</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1.5fr', gap: 40, marginBottom: 48 }} className="footer-g">
            <div>
              ...
              <p className="mono" style={{ fontSize: 12, color: 'var(--cinza)', lineHeight: 1.7, marginTop: 16, maxWidth: '36ch' }}>
                Curvas calculadas.<br/>Preços legíveis.<br/>Decisões menos assimétricas.
              </p>
            </div>
            <div>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8, fontSize: 13, color: 'var(--cinza)' }}>
                <li><Link href="/curva">Curva dólar forward</Link></li>
                <li><Link href="/juros">Curva de juros</Link></li>
                <li><Link href="/ndf">NDF</Link></li>
                <li><Link href="/comparador">Comparador</Link></li>
              </ul>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mono" style={{ fontSize: 11, color: 'var(--cinza-2)', lineHeight: 1.8 }}>
                Não constitui recomendação individual de investimento. Cálculos são estimativas baseadas em premissas informadas.
              </p>
            </div>
          </div>
